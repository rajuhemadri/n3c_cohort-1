...
select * from 
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as asian from enclave_cohort.severity_table2_for_export where value='Asian') as asian
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as black from enclave_cohort.severity_table2_for_export where value='Black or African American') as black
	on true
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as unknown from enclave_cohort.severity_table2_for_export where value='Missing/Unknown' and variable='Race') as unknown
	on true
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as hawaiian from enclave_cohort.severity_table2_for_export where value='Native Hawaiian or Other Pacific Islander') as hawaiian
	on true
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as other from enclave_cohort.severity_table2_for_export where value='Other' and variable='Race') as other
	on true
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as white from enclave_cohort.severity_table2_for_export where value='White') as white
	on true
</sql:query>
