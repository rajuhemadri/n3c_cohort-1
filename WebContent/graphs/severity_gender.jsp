...
select * from 
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as female from enclave_cohort.severity_table2_for_export where value='FEMALE') as female
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as male from enclave_cohort.severity_table2_for_export where value='MALE') as mail
	on true
	left join
	(select to_char(substring(x__all::text from '[0-9]+')::int, '999,999,999') as other from enclave_cohort.severity_table2_for_export where value='Other' and variable='gender_concept_name') as other
	on true
</sql:query>
